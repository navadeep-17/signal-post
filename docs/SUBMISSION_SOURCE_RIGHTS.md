...
This document describes the **production submission path** used by `scripts/run_signalpost_final.py`. Experimental scripts elsewhere in the repository are not enabled by that runner and must not be interpreted as submission sources.
...
|---|---|---|---|---|---|
| Brønnøysundregistrene entity bulk/live API | exact identity anchor, legal name/form, municipality, employee count, latest-account metadata | Official bulk download and official REST API | BRREG open-data datasets are published under NLOD 2.0. Attribution: Brønnøysundregistrene. | normalized facts plus source URL, retrieval time, hash/evidence metadata | 404/absence is not converted to zero; org number remains the anchor |
| BRREG roles/group/subunits | leadership roles, group relationships, registered establishments | Official REST APIs | BRREG open-data datasets under NLOD 2.0 | company-centric role/location/group facts and provenance | birth dates are discarded; no unrelated person profile is built |
...
| Verified company-owned website | official-site proof, bounded description, company-declared social URLs, same-domain contact email | bounded public HTTP retrieval with safe URL/redirect handling and robots behavior | No blanket content-reuse licence is assumed | source URL, retrieval time, hash, bounded factual claim span, normalized outbound URL/email | exact-company proof required; ambiguous/wrong-entity pages abstain; full pages are not republished |
...
The optional sentiment dependencies and experimental sentiment scripts in the repository are not enabled by `scripts/run_signalpost_final.py` and produce no certified release claims.
...
The certified 1,000-company replay observed 13,628 conservative requests across ten 100-company chunks and zero search-API requests.
...
No external hosting is required to execute or inspect the submission. The evidence workspace is a static HTML artifact generated locally from final output-contract JSONL. It can be opened directly in a browser.

## Explicitly excluded from the certified production stack

The following experiments or ideas are not production submission sources:
...
Historical design documents may discuss these experiments. `SUBMISSION.md`, `submission/manifest.json`, this file and `docs/FINAL_RELEASE_1000_AUDIT.md` are authoritative for the final submission path.

...
| SerpApi Google Search API | `run_search_discovery.py`, `discovery.py`, manual `h1b-search-discovery.yml` | EXPERIMENTAL / RIGHTS REVIEWED / LIVE AUDIT PENDING | Public terms/legal docs reviewed 2026-09-14 (terms updated 2026-08-27). No Brave-style prohibition on AI evaluation/benchmarking was found in the reviewed public ToS; SerpApi documents 31-day search-data retention and Enterprise ZeroTrace. Treat provider results as transient crawl-candidate hints only: persist query hash/provider metadata, discard title/snippet/rank/raw response, independently fetch the candidate, and require H1b page-level exact-company proof before publication. Live benchmark remains disabled until `SERPAPI_API_KEY` and an explicit `SERPAPI_COST_PER_SEARCH_USD` are configured and promotions are manually audited. Do not interpret this status as legal advice or final source qualification. |
| Tavily search | no promoted production integration | PLANNED / RIGHTS REVIEW | Current terms support Customer Applications/AI use but include restrictions around disclosure of performance information about Tavily Services. Do not use for a public H1b benchmark without clearer written confirmation or plan terms. |

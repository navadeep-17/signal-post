...
def _evaluate_one(profile: dict[str, Any], *, timeout: float) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    ...
    row, metrics = evaluate_hyphenated_no_fallback(profile, timeout=timeout)
...
def _audit_row(profile: dict[str, Any], metrics: dict[str, Any], added: list[dict[str, Any]]) -> dict[str, Any]:
    candidate = ((profile.get("evidence") or {}).get("website_h1g_candidate") or {})
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate a bounded hyphenated legal-name .no fallback using only unused production request headroom."
    )
    ...
    audits = [_audit_row(profile, metric, added) for profile, metric, added in ordered if metric.get("attempted")]
    ...
    report = {
        ...
        "request_budget": {
            ...
            "post_h1g_profile_logical_requests": post_profile_logical,
            "shared_wikidata_logical_requests": shared_logical,
            "post_h1g_observed_logical_requests": post_observed_logical,
            "request_charge_multiplier": charge_multiplier,
            "post_h1g_conservative_challenge_request_charge": post_conservative_charge,
